 ...
 * v5.2 — modal con listado de alumnos anotados
...
(async function () {
  ...
  function inyectarEstilosGlobales() {
    ...
    s.textContent = `
      ...
      .cfy-clase-card {
        ...
        border: 1px solid var(--cfy-border); border-radius: 4px;
        cursor: pointer; transition: border-color .15s, background .15s;
        background: transparent; width: 100%; text-align: left;
      }
      .cfy-clase-card:hover { border-color: var(--cfy-blue); background: rgba(64,196,255,.05); }
      .cfy-clase-hora { color:var(--cfy-green); font-weight:700; font-size:13px; min-width:40px; flex-shrink:0; }
      ...
      .cfy-dia-header {
        font-family:var(--cfy-font-ui); font-size:10px; font-weight:700;
        letter-spacing:.12em; text-transform:uppercase; color:var(--cfy-muted);
        padding:8px 0 5px; border-bottom:1px solid var(--cfy-border); margin-bottom:6px;
      }
      .cfy-dia-header:first-child { padding-top:0; }

      .cfy-spinner {
        width:16px; height:16px; border:2px solid var(--cfy-border);
        border-top-color:var(--cfy-green); border-radius:50%;
      ...
      .cfy-modal-header {
        padding:14px 20px 12px; border-bottom:1px solid var(--cfy-border);
      ...
      /* ── Sección alumnos ── */
      .cfy-alumnos-section {
        border-top: 1px solid var(--cfy-border);
      ...
      .cfy-alumno-row:last-child { border-bottom: none; }
      .cfy-alumno-num {
        color: var(--cfy-muted);
        font-size: 9px;
        min-width: 16px;
      ...
      .cfy-tab {
        ...
        border: 1px solid var(--cfy-border); border-radius: 3px;
        color: var(--cfy-muted); font-family: var(--cfy-font-ui);
        font-size: 10px; font-weight: 700; letter-spacing: .08em;
        text-transform: uppercase; cursor: pointer;
        transition: color .15s, border-color .15s, background .15s;
      }
      .cfy-tab:hover { color: var(--cfy-text); border-color: var(--cfy-muted); }
      .cfy-tab.active {
        color: var(--cfy-green); border-color: var(--cfy-green);
        background: rgba(0,230,118,.06);
      }
      .cfy-tab-content { display: none; flex-direction: column; flex: 1; min-height: 0; overflow-y: auto; }
      .cfy-tab-content.active { display: flex; }

      /* ── Drag / Minimize ── */
      .cfy-drag-handle { cursor: grab; user-select: none; }
      .cfy-drag-handle:active { cursor: grabbing; }
      .cfy-btn-min {
        background: transparent; border: none; color: var(--cfy-muted);
        cursor: pointer; font-family: var(--cfy-font-mono); font-size: 16px;
        line-height: 1; padding: 0 4px; flex-shrink: 0;
      }
      .cfy-btn-min:hover { color: var(--cfy-text); }
      .cfy-panel.minimized .cfy-tab-bar,
      .cfy-panel.minimized .cfy-tab-content { display: none !important; }
    `;
  ...
  const AUTH_CACHE_TTL_MS = 4 * 60 * 60 * 1000; // 4 horas
  ...
  function labelDia(fecha) {
    ...
    if (esManana) return `MAÑANA · ${formatearFechaDDMMYYYY(fecha)}`;
  ...
  function _renderLog() {
    ...
    el.innerHTML = logEntries.slice().reverse().map(e =>
      `<div class="cfy-log-entry">
        ...
        <span class="cfy-log-msg ${e.tipo}">${escapeHtml(e.msg)}</span>
      </div>`
    ).join("");
  }
  function notificarCritico(msg, tipo = "success") {
    ...
    toast.innerHTML = `
      ...
      <span class="cfy-toast-critical-msg">${escapeHtml(msg)}</span>
    ...
    if (clase) {
      ...
      claseHtml = `
        <div style="margin-bottom:10px;padding-bottom:10px;border-bottom:1px solid var(--cfy-border)">
          ...
          <div style="color:#fff;font-size:14px;font-weight:700">${escapeHtml(nombre)}</div>
    ...
    if (!h.length) {
      ...
      inner += h.map(i => `
        <div class="cfy-hist-item">
          <div>
            <div style="color:var(--cfy-text);font-size:10px;opacity:.7">${escapeHtml(i.nombreDisciplina ?? "")}</div>
  ...
  /**
   * Carga el detalle de la clase usando buscarPorId.
   * Respuesta: { clase, turno, disponibles, enListaDeEspera, anotados: [{id, nombreCompleto}] }
   */
  function fetchAlumnos(idClase, _fechaClase, callback) {
    crossfy.service.clases.buscarPorId(idClase, {
      successCallback(detalle) {
        const alumnos = (detalle?.anotados ?? []).map(a => a.nombreCompleto?.trim() ?? "—");
        callback(null, alumnos, detalle);
      },
      errorCallback(xhr) {
        callback(parseApiError(xhr, "No se pudo cargar la lista de alumnos."), [], null);
      },
    });
  }
  ...
  function iniciarBotConClase(modo, claseSel) {
    ...
    actualizarEstadoPanelPrincipal(true);
    switchTab('estado');
    ...
    if (modo === MODO_ATAQUE) {
      ...
      const inicioReal    = new Date(inicioTeorico.getTime() - 15 * MS_SEG);
  ...
  // ---------------------------------------------------------------------------
  // PANEL PRINCIPAL — listado de clases HOY + MAÑANA
  // ---------------------------------------------------------------------------
  function cargarClases() {
    ...
    const pManana = formatearFechaDDMMYYYY(manana);

    let resHoy = null, resManana = null, errHoy = false, errManana = false;

    const intentarRender = () => {
      if ((resHoy !== null || errHoy) && (resManana !== null || errManana)) {
        renderClases(listEl, [
          { label: labelDia(hoy),    clases: resHoy    ?? [], error: errHoy },
          { label: labelDia(manana), clases: resManana ?? [], error: errManana },
        ]);
      }
    };
    ...
    crossfy.service.clases.getClases(null, pManana, {
      successCallback(data) { resManana = data?.[0]?.clases ?? []; intentarRender(); },
      errorCallback()       { errManana = true; intentarRender(); },
    });
  }
      ...
      clases.forEach(c => {
        ...
        card.innerHTML = `
          ...
          <span class="cfy-clase-info">
            <span class="cfy-clase-disc">${escapeHtml(nombre)}</span>
          </span>
        ...
        card.addEventListener("click", () => {
          confirmarModo({ id: c.id, fecha: f, nombre, nombreDisciplina: nombre });
        });
  ...
  // ---------------------------------------------------------------------------
  // MODAL DE CONFIRMACIÓN DE MODO — con listado de alumnos
  // ---------------------------------------------------------------------------
  function confirmarModo(clase) {
    ...
    overlay.innerHTML = `
      <div class="cfy-modal">
        ...
        <div class="cfy-modal-clase">
          <div style="color:#fff;font-size:16px;font-weight:700">${hora} · ${escapeHtml(clase.nombreDisciplina)}</div>
        ...
        <div class="cfy-alumnos-section">
          <div class="cfy-alumnos-header">
            <span class="cfy-alumnos-title">Alumnos anotados</span>
            <span class="cfy-alumnos-count" id="cfy-alumnos-count">—</span>
          </div>
          <div id="cfy-alumnos-lista" class="cfy-alumnos-lista">
            <div class="cfy-alumnos-loading">
              <span class="cfy-spinner"></span>Cargando…
            </div>
          </div>
        </div>
      </div>`;
    ...
    // Cargar alumnos de forma asíncrona — el modal ya está visible
    fetchAlumnos(clase.id, clase.fecha, (error, alumnos, detalle) => {
      const listaEl = document.getElementById("cfy-alumnos-lista");
      const countEl = document.getElementById("cfy-alumnos-count");
      if (!listaEl) return; // modal cerrado antes de que llegara la respuesta

      if (error) {
        listaEl.innerHTML = `<div class="cfy-alumnos-error">⚠ ${escapeHtml(error)}</div>`;
        if (countEl) countEl.textContent = "—";
        return;
      }

      // Mostrar "X / capacidad" si tenemos el detalle completo
      if (countEl) {
        const capacidad = detalle?.clase?.capacidad;
        countEl.textContent = capacidad
          ? `${alumnos.length} / ${capacidad}`
          : `${alumnos.length}`;
      }

      if (!alumnos.length) {
        listaEl.innerHTML = `<div class="cfy-alumnos-empty">Sin alumnos anotados aún</div>`;
        return;
      }

      listaEl.innerHTML = alumnos.map((nombre, i) => `
        <div class="cfy-alumno-row">
          <span class="cfy-alumno-num">${i + 1}</span>
          <span class="cfy-alumno-nombre">${escapeHtml(nombre)}</span>
        </div>`).join("");
    });
  }

  // ---------------------------------------------------------------------------
  // ESTADO PANEL PRINCIPAL
  // ---------------------------------------------------------------------------
  ...
  // CREAR UI COMPLETA
    function switchTab(name) {
      ...
      panelPrincipal.querySelectorAll('.cfy-tab').forEach(t => t.classList.toggle('active', t.dataset.tab === name));
      panelPrincipal.querySelectorAll('.cfy-tab-content').forEach(c => c.classList.toggle('active', c.id === `cfy-tab-${name}`));
    }
  ...
  function crearUI() {
    ...
    panelPrincipal.style.cssText = "top:20px;right:20px;width:280px;padding:14px;max-height:calc(100vh - 40px);min-height:320px;display:flex;flex-direction:column;";

    panelPrincipal.innerHTML = `
      <div class="cfy-drag-handle" id="cfy-drag-handle"
        style="display:flex;align-items:center;gap:8px;margin-bottom:12px;padding-bottom:12px;border-bottom:1px solid var(--cfy-border);flex-shrink:0">
        ...
        <div style="flex:1">
          ...
          <div style="font-size:9px;color:var(--cfy-muted);letter-spacing:.08em">v5.2 · AUTO-RESERVE</div>
        </div>
        <button class="cfy-btn-min" id="cfy-btn-minimize" title="Minimizar">─</button>
      </div>

      <div class="cfy-tab-bar">
        <button class="cfy-tab active" data-tab="clases">CLASES</button>
        <button class="cfy-tab"        data-tab="estado">ESTADO</button>
        <button class="cfy-tab"        data-tab="historial">HISTORIAL</button>
      </div>

      <div class="cfy-tab-content active" id="cfy-tab-clases">
      ...
      <div class="cfy-tab-content" id="cfy-tab-estado"></div>
      <div class="cfy-tab-content" id="cfy-tab-historial"></div>`;

    document.body.appendChild(panelPrincipal);

    // ── Tabs ──
    panelPrincipal.querySelectorAll('.cfy-tab').forEach(tab => {
      tab.addEventListener('click', () => {
        panelPrincipal.querySelectorAll('.cfy-tab').forEach(t => t.classList.remove('active'));
        panelPrincipal.querySelectorAll('.cfy-tab-content').forEach(c => c.classList.remove('active'));
        tab.classList.add('active');
        document.getElementById(`cfy-tab-${tab.dataset.tab}`).classList.add('active');
      });
    });

    // ── Minimize ──
    panelPrincipal.querySelector('#cfy-btn-minimize').addEventListener('click', () => {
      panelPrincipal.classList.toggle('minimized');
    });

    // ── Drag ──
    const dragHandle = panelPrincipal.querySelector('#cfy-drag-handle');
    let dragging = false, dragOffX = 0, dragOffY = 0;
    dragHandle.addEventListener('mousedown', e => {
      if (e.target.closest('#cfy-btn-minimize')) return;
      dragging = true;
      const rect = panelPrincipal.getBoundingClientRect();
    ...
    document.addEventListener('mousemove', e => {
      if (!dragging) return;
      panelPrincipal.style.left   = `${e.clientX - dragOffX}px`;
      panelPrincipal.style.top    = `${e.clientY - dragOffY}px`;
      panelPrincipal.style.right  = 'auto';
      panelPrincipal.style.bottom = 'auto';
    });
    ...
    // ── Asignar subpaneles a los tab divs ──
    panelEstado    = document.getElementById('cfy-tab-estado');
    panelHistorial = document.getElementById('cfy-tab-historial');
